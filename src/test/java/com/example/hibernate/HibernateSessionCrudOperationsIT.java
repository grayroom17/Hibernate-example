...
@Slf4j
class HibernateSessionCrudOperationsIT extends BaseIT {
    ...
    void persist_whenDataIsSavedButNotPersisted_thenThrowConstraintViolationException() {
        try (var session = sessionFactory.openSession()) {
            ...
            var transaction = session.beginTransaction();
            var message = "Converting `org.hibernate.PersistentObjectException` to JPA `PersistenceException` : detached entity passed to persist: com.example.hibernate.entity.User";
            ...
            var causeMessage = "detached entity passed to persist: com.example.hibernate.entity.User";
    ...
    void persist_whenDataIsAlreadyPersisted_thenThrowEntityExistsException() {
        try (var session = sessionFactory.openSession()) {
            ...
            var transaction = session.beginTransaction();
            var exceptionMessage = "A different object with the same identifier value was already associated with the session : [com.example.hibernate.entity.UserForOneToManyTests#defaultUser]";
    ...
    void remove_whenDataIsAlreadyPersisted_thenThrowsEntityExistsException() {
        try (var session = sessionFactory.openSession()) {
            ...
            var transaction = session.beginTransaction();
            var message = "A different object with the same identifier value was already associated with the session : [com.example.hibernate.entity.UserForOneToManyTests#userMustBeDeleted]";
